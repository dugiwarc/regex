function processData(input) {
	...
	let regPattern =
		"https?://(?:ww[a-zA-Z0-9_-]+\\.)?([a-zA-Z0-9_-]+(?:\\.[a-zA-Z0-9_-]+)+)[/?\"']";

	let regExp = new RegExp(regPattern, "ig");
	let matches = input.match(regExp);
	let arrayRes = [];
	for (let i = 0; i < matches.length; i++) {
		let domain = matches[i];
		if (domain.length > 0) {
			const result = domain.replace(
				/https?:\/\/(www)?(\.)?|[/\"?']/gi,
				"",
			);
			if (!arrayRes.includes(result)) arrayRes.push(result);
		}
	}

	process.stdout.write(arrayRes.sort().join(";") + "\n");
}
